...
object CoreStatisticsDaily extends CustomParams {

  val getBucketBorder = udf((bucket: Double, labels: Seq[String]) => labels(bucket.toInt));

  val q_sdf = spark.table(TableCoreBordersDaily)

  val sdf = spark.table(TableCoreModelScoring)
    .select(
      to_date(col("sale_dt")).as("sale_dt"),
      col("pred_calibrated"),
      col("subs_key")
    );

  val start_date = sdf
    .selectExpr("date_sub(max(sale_dt), 93)")
    .first()
    .getDate(0);

  val sdf_filtered = sdf
    .filter(col("sale_dt") >= start_date);
  val states = q_sdf
    .select(col("q"))
    .rdd
  ...
  bucketedWithBorders
    ...
    .format("orc")
    .mode("overwrite")
    .insertInto(TableCoreStatisticsDaily)
